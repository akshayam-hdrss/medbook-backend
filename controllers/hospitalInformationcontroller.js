...
const [result] = await db.query(
  ...
  [
    ...
    res.status(201).json({ message: "Hospital information created", id: result.insertId });
  } catch (error) {
    console.error(error);
    res.status(500).json({ error: "Failed to create hospital information" });
  }
};

// READ ALL
exports.getHospitalInformation = async (req, res) => {
  try {
    const [rows] = await db.query("SELECT * FROM hospitalInformation");
    res.json(rows);
  } catch (error) {
    console.error(error);
...
exports.getHospitalInformationByHospitalId = async (req, res) => {
  try {
    const { hospitalId } = req.params;
    const [rows] = await db.query("SELECT * FROM hospitalInformation WHERE hospitalId = ?", [hospitalId]);
...
// UPDATE (by hospitalId)
exports.updateHospitalInformation = async (req, res) => {
  try {
    const { hospitalId } = req.params;
    const {
      specialties,
    ...
    res.json({ message: "Hospital information updated" });
  } catch (error) {
    console.error(error);
    res.status(500).json({ error: "Failed to update hospital information" });
  }
};
...
exports.deleteHospitalInformation = async (req, res) => {
  try {
    const { hospitalId } = req.params;
    const [result] = await db.query("DELETE FROM hospitalInformation WHERE hospitalId = ?", [hospitalId]);

    if (result.affectedRows === 0) {
      return res.status(404).json({ message: "Hospital information not found" });
    }

    res.json({ message: "Hospital information deleted" });
  } catch (error) {
    console.error(error);
    res.status(500).json({ error: "Failed to delete hospital information" });
  }
};